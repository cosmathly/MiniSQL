...
bool Catalog_File::check_table_if_exist(std::string table_name) 
{
     ...
     Table_Info *cur_table_info = new Table_Info;
     int ret;
     char *buf;
     while(offset_file<table_info.offset_file)
     {      
           
           ret = buffer->Read(table_info.fd, offset_file, 0, &buf);
           if(ret==-1) return false;
                    for(int i = 0; i < Block_Size; i++)
         if((*(buf+i))=='\n')
         {
            memcpy(cur_table_info, buf, i);
            break;
         }
...
{
     offset_t offset_file = 0;
     Index_Info *cur_index_info = new Index_Info;
     int ret;
     char *buf;
     while(offset_file<index_info.offset_file)
     {     
           ret = buffer->Read(index_info.fd, offset_file, 0, &buf);
           if(ret==-1) return false;
                    for(int i = 0; i < Block_Size; i++)
         if((*(buf+i))=='\n')
         {
            memcpy(cur_index_info, buf, i);
            break;
         }
           offset_file += Block_Size;
...
bool Catalog_File::check_attr_if_exist(std::string table_name, std::string attr_name) 
{
     offset_t offset_file = 0;
     int ret;
     Attr_Info *cur_attr_info = new Attr_Info;
     char *buf;
     while(offset_file<attr_info.offset_file)
     {         
           ret = buffer->Read(attr_info.fd, offset_file, 0, &buf);
           if(ret==-1) return false;
                    for(int i = 0; i < Block_Size; i++)
         if((*(buf+i))=='\n')
         {
            memcpy(cur_attr_info, buf, i);
            break;
         }
           offset_file += Block_Size;
...
void Catalog_File::insert_table_info(Table_Info *src_table_info)
{
     ...
     int ret = buffer->Write(table_info.fd, table_info.offset_file, 0, src_table_info, sizeof(*src_table_info));
     table_info.offset_file += Block_Size;
}
void Catalog_File::insert_attr_info(Attr_Info *src_attr_info)
{
     buffer->Write(attr_info.fd, attr_info.offset_file, 0, src_attr_info, sizeof(*src_attr_info));
     attr_info.offset_file += Block_Size;
}
void Catalog_File::insert_index_info(Index_Info *src_index_info)
{
     buffer->Write(index_info.fd, index_info.offset_file, 0, src_index_info, sizeof(*src_index_info));
...
Attr_Set * Catalog_File::get_all_index_attr(std::string table_name) 
{
           ...
           while(offset_file<index_info.offset_file)
           { 
                  int ret = buffer->Read(index_info.fd, offset_file, 0, &buf);
                  if(ret==-1) break;
                  for(int i = 0; i < Block_Size; i++)
         if((*(buf+i))=='\n')
         {
...
All_Attr * Catalog_File::get_all_attr(std::string table_name) 
{
            ...
            while(offset_file<attr_info.offset_file)
            { 
                  int ret = buffer->Read(attr_info.fd, offset_file, 0, &buf);
                  if(ret==-1) break;
                  for(int i = 0; i < Block_Size; i++)
         if((*(buf+i))=='\n')
         {
...
void Catalog_File::drop_table(std::string table_name)
{
         ...
         if((*(buf+i))=='\n')
         {
            ...
            break;
                  ...
                  buffer->Write(table_info.fd, offset_file-Block_Size, 0, cur_table_info, sizeof(*cur_table_info));
            }
}
void Catalog_File::drop_attr(std::string table_name)
{
         ...
         if((*(buf+i))=='\n')
         {
            ...
            break;
                  ...
                  buffer->Write(attr_info.fd, offset_file-Block_Size, 0, cur_attr_info,  sizeof(*cur_attr_info));
            }
}
void Catalog_File::drop_index(std::string index_name)
{
         ...
         if((*(buf+i))=='\n')
         {
            ...
            break;
                  ...
                  buffer->Write(index_info.fd, offset_file-Block_Size, 0, cur_index_info, sizeof(*cur_index_info));   
           }
}
